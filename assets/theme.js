...
`;
  ...
  var ProductLoader = class {
    static load(productHandle) {
      ...
      this.loadedProducts[productHandle] = new Promise(async (resolve) => {
        ...
        hideVariants.forEach(hideVariant => {
          var index = 0;
          responseAsJson.variants.forEach(currentVariant => {
            if(currentVariant.id == parseInt(hideVariant)) {
              //console.log(hideVariant);
              responseAsJson.variants.splice(index, 1);
            }
            index++;
          });
        });
  ...
  var ProductRecommendations = class extends HTMLElement {
    ...
    get recommendationsCount() {
      return parseInt(this.getAttribute("recommendations-count") || 10);
    }
  };
  ...
  var CustomProductRecommendations = class extends HTMLElement {
    async connectedCallback() {
      ...
      const response = await fetch(`${window.themeVariables.routes.productRecommendationsUrl}?product_id=${lastViewed}&limit=${this.recommendationsCount}&section_id=${this.sectionId}`);
      const div = document.createElement("div");
      div.innerHTML = await response.text();
      const productRecommendationsElement = div.querySelector("custom-recommendations");
      if (productRecommendationsElement.hasChildNodes()) {
        this.innerHTML = productRecommendationsElement.innerHTML;
      }
    }
  ...
  window.customElements.define("custom-recommendations", CustomProductRecommendations);
  ...
  var ProductVariants = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      $('.product-form__option-selector').removeAttr('style');
